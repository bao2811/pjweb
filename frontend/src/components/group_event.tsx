...
const mockEvents: Record<string, Event> = {
  "1": {
    ...
    description: "Cùng nhau trồng cây tại công viên để tạo ra không gian xanh cho cộng đồng",
...
const mockPostsByEvent: Record<string, Post[]> = {
  ...
  "2": [
    ...
    },
    {
      id: 2,
      eventId: "2",
      content: "Thông báo lịch trình: Ngày 1 - Di chuyển và làm quen. Ngày 2 - Dạy học buổi sáng, hoạt động ngoài trời buổi chiều. Ngày 3 - Trao quà và chia tay các em 🎒",
...
export default function Group({ eventId, role = "user" }: GroupProps) {
  ...
  const [postFilter, setPostFilter] = useState<"all" | "organizer" | "media" | "myposts">("all");
  const [messages, setMessages] = useState<ChatMessage[]>([]);
  const [resources, setResources] = useState<Resource[]>(mockResources[eventId] || []);
  ...
  const [activityUsers] = useState<ActivityUser[]>(mockActivityUsers[eventId] || []);
  ...
  useEffect(() => {
    const fetchCurrentUser = async () => {
      ...
      console.log('🔑 JWT Token:', token ? `${token.substring(0, 50)}...` : 'NO TOKEN');
      ...
      try {
        ...
        // Handle cả 2 trường hợp: response.data.user hoặc response.data trực tiếp
        ...
        if (userData && userData.id) {
          ...
          console.warn('⚠️ Invalid user data structure:', response.data);
          setCurrentUserData(currentUser);
        }
      } catch (error) {
    ...
    const fetchEventDetail = async () => {
      try {
        ...
        if (response.data && response.data.event) {
          ...
          try {
            const channelResponse = await api.get(`events/${eventId}/channel`);
            console.log('📡 Channel Response:', channelResponse.data);
            if (channelResponse.data && channelResponse.data.channel) {
              const fetchedChannelId = channelResponse.data.channel.id;
              console.log(`✅ Event ID: ${eventId} → Channel ID: ${fetchedChannelId}`);
        ...
        if (postsData && Array.isArray(postsData)) {
          const normalizedPosts: Post[] = postsData.map((post: any) => ({
            ...
            author: {
              ...
              avatar: post.user?.avatar || "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=150&h=150&fit=crop",
            ...
            comments: (post.comments || []).map((c: any) => ({
              ...
              author: {
                ...
                avatar: c.user?.avatar || c.author?.avatar || "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=150&h=150&fit=crop",
              ...
              replies: (c.replies || []).map((r: any) => ({
                ...
                author: {
                  ...
                  avatar: r.user?.avatar || r.author?.avatar || "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=150&h=150&fit=crop",
          ...
          console.log('🔍 Posts with likes:', normalizedPosts.map(p => ({ id: p.id, likes: p.likes, isLiked: p.isLiked })));
          setPosts(normalizedPosts);
        } else {
          console.warn('⚠️ Invalid posts response format:', response.data);
        }
      } catch (error) {
    ...
    const fetchMessages = async () => {
      ...
      try {
        ...
        if (response.data && Array.isArray(response.data)) {
          const normalizedMessages: ChatMessage[] = response.data.map((msg: any) => ({
            id: msg.id,
            userId: msg.sender_id,
            userName: msg.sender?.name || "User",
            userAvatar: msg.sender?.avatar || "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=150&h=150&fit=crop",
            message: msg.content,
            timestamp: new Date(msg.sent_at).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' }),
  ...
  const filteredPosts = posts.filter(post => {
    if (postFilter === "organizer") return post.author.role === "manager" || post.author.role === "admin";
  ...
  const todayMessages = messages.filter(m => m.timestamp.includes("giờ") || m.timestamp.includes("phút")).length;
  const totalResources = resources.length;
  const activityScore = Math.round((totalPosts * 10 + todayMessages * 5 + totalResources * 15) / 10);
  ...
  const handlePost = async () => {
    ...
    try {
      const response = await api.post('posts/channel', {
    ...
    setPosts(posts.map(post => {
      if (post.id === postId) {
        ...
        if (existingReaction) {
          ...
          return {
            ...post,
            reactions: [...reactions, { type: reactionType, count: 1, users: [] }]
          };
        }
      }
      return post;
  ...
  const handleLike = async (postId: number) => {
    ...
    try {
      if (post.isLiked) {
        await api.post(`likes/post/unlike/${postId}`);
      } else {
        await api.post(`likes/post/like/${postId}`);
      }
    } catch (error) {
  ...
  const handleComment = async (postId: number, parentCommentId?: number) => {
    ...
    console.log('💬 Creating comment:', { postId, parentCommentId, content: comment });
    ...
    try {
      const response = await api.post('/posts/addCommentOfPost', {
        post_id: postId,
        content: comment,
      ...
      if (response.data.comment) {
        const serverComment: Comment = {
          ...
          timestamp: new Date(response.data.comment.created_at).toLocaleString('vi-VN'),
          author: {
            id: response.data.comment.author.id,
            name: response.data.comment.author.name,
            avatar: response.data.comment.author.avatar,
            role: response.data.comment.author.role
          },
          likes: 0,
          isLiked: false,
          replies: [],
            ...
            if (parentCommentId) {
              const updatedComments = post.comments.map(c => {
                if (c.id === parentCommentId) {
                  return {
                    ...c,
                    replies: c.replies.map(r => r.id === tempId ? serverComment : r)
                  };
                }
                return c;
              });
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 flex items-center justify-center">
        <div className="text-center bg-white rounded-2xl shadow-xl p-12">
          ...
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Không tìm thấy sự kiện</h1>
          <p className="text-gray-600">Sự kiện với ID "{eventId}" không tồn tại hoặc đã bị xóa.</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3 bg-white/60 backdrop-blur-sm rounded-xl px-4 py-2 shadow-sm">
                ...
                />
                <div>
                  <h1 className="text-lg font-bold text-gray-900 line-clamp-1">{event.title}</h1>
          ...
          {activeTab === "posts" && (
            <div className="space-y-5">
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 text-left px-5 py-3 bg-gray-50 hover:bg-gray-100 rounded-full transition-colors">
                    <p className="text-gray-500">{currentUserData.name} ơi, bạn đang nghĩ gì về sự kiện này?</p>
                  </div>
                </div>
              </button>
              ...
              {filteredPosts.map((post) => (
                <div key={post.id} className={`bg-white rounded-2xl shadow-md border overflow-hidden 
                                            hover:shadow-xl transition-all duration-300 ${
                                              post.isPinned ? 'border-green-400 border-2' : 'border-gray-200'
                                            }`}>
                  {/* Pinned Badge */}
                  {post.isPinned && (
                  ...
                  <div className="p-5">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {(role === "admin" || post.author.id === currentUserData.id) && (
                        ...
                        {post.comments.map((comment) => (
                          <div key={comment.id} className="space-y-3">
                            ...
                            <div className="flex gap-3 items-start">
                              <button
                                onClick={() => router.push(`/user/profile/${comment.author.id}`)}
                                className="flex-shrink-0 hover:opacity-80 transition"
                              >
                                <Image
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="bg-white rounded-2xl px-4 py-3 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                                  <div className="flex items-start justify-between gap-2 mb-1">
                                    ...
                                    <span className="text-xs text-gray-400 whitespace-nowrap">{comment.timestamp}</span>
                                  </div>
                                ...
                                <div className="flex items-center gap-4 mt-2 ml-1">
                                  ...
                                  >
                                    {comment.isLiked ? <FaHeart className="w-3.5 h-3.5" /> : <FaRegHeart className="w-3.5 h-3.5" />}
                                    {comment.likes > 0 && <span>{comment.likes}</span>}
                                  </button>
                                  <button 
                                    onClick={() => {
                                      const commentKey = `${post.id}-${comment.id}`;
                                      setShowComments({...showComments, [commentKey]: !showComments[commentKey]});
                                    }}
                                  ...
                                  <div className="flex gap-2 mt-3 pl-1">
                                    ...
                                    <div className="flex-1 flex gap-2">
                                      <input
                                        ...
                                        onKeyPress={(e) => e.key === "Enter" && !e.shiftKey && handleComment(post.id, comment.id)}
                                      />
                                      <button
                                        onClick={() => handleComment(post.id, comment.id)}
                                        disabled={!newComment[`${post.id}-${comment.id}`]?.trim()}
                                  ...
                                  <div className="ml-10 mt-3 space-y-3 border-l-2 border-blue-100 pl-4">
                                    {comment.replies.map((reply) => (
                                      <div key={reply.id} className="flex gap-2 items-start">
                                        <button
                                          onClick={() => router.push(`/user/profile/${reply.author.id}`)}
                                          className="flex-shrink-0 hover:opacity-80 transition"
                                        >
                                          <Image
                                        ...
                                        <div className="flex-1 min-w-0">
                                          <div className="bg-gray-50 rounded-xl px-3 py-2 border border-gray-100 hover:bg-gray-100 transition-colors">
                                            <div className="flex items-start justify-between gap-2 mb-1">
                                              ...
                                              <span className="text-xs text-gray-400 whitespace-nowrap">{reply.timestamp}</span>
                                            </div>
                                          ...
                                          <div className="flex items-center gap-3 mt-1.5 ml-1">
                                            ...
                                            >
                                              {reply.isLiked ? <FaHeart className="w-3 h-3" /> : <FaRegHeart className="w-3 h-3" />}
                                              {reply.likes > 0 && <span>{reply.likes}</span>}
                                            </button>
                                            <button 
                                              onClick={() => {
                                                const commentKey = `${post.id}-${comment.id}`;
                                                setShowComments({...showComments, [commentKey]: true});
                                              }}
                                              className="text-xs font-medium text-gray-500 hover:text-blue-600 transition-colors"
                                            >
                                              Trả lời
                ...
                <div className="h-[calc(100%-5rem)] overflow-y-auto p-6 space-y-3 bg-gradient-to-br from-green-50/30 to-blue-50/30">
                  {messages.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-center">
                      ...
                      <h3 className="font-bold text-gray-700 text-lg mb-2">Chưa có tin nhắn</h3>
                      <p className="text-gray-500">Bắt đầu cuộc trò chuyện với các thành viên!</p>
                    </div>
                  ) : (
                    <>
            ...
            <div className="space-y-4 max-w-4xl mx-auto">
              ...
              {(role === "manager" || role === "admin") && (
                <div className="bg-gradient-to-r from-green-50 to-blue-50 rounded-xl shadow-md border-2 border-dashed border-green-300 p-8 text-center">
                  ...
                  <h3 className="font-bold text-gray-900 mb-2">Tải lên tài liệu</h3>
                  <p className="text-gray-600 text-sm mb-4">Thêm tài liệu, hình ảnh hoặc video hữu ích cho sự kiện</p>
              ...
              {resources.length === 0 ? (
                <div className="bg-white rounded-xl shadow-md border border-gray-200 p-16 text-center">
                  ...
                  <h3 className="text-lg font-semibold text-gray-700 mb-2">Chưa có tài liệu</h3>
                  <p className="text-gray-500">Ban tổ chức sẽ cập nhật tài liệu sớm nhất</p>
                </div>
              ) : (
                <div className="grid gap-4">
                  {resources.map((resource) => (
                    <div key={resource.id} className="bg-white rounded-xl shadow-md border border-gray-200 p-5 hover:shadow-lg transition">
                      <div className="flex items-center gap-4">
                        {/* File Icon */}
                        <div className={`w-14 h-14 rounded-xl flex items-center justify-center ${
                          resource.type === "pdf" ? "bg-red-100" :
                          resource.type === "doc" ? "bg-blue-100" :
                          resource.type === "image" ? "bg-purple-100" :
                          ...
                          <FaFileAlt className={`text-2xl ${
                            ...
                            resource.type === "image" ? "text-purple-600" :
                            resource.type === "video" ? "text-green-600" :
                            "text-gray-600"
                          }`} />
                        </div>
                        ...
                        {/* File Info */}
                        <div className="flex-1 min-w-0">
                          <h4 className="font-semibold text-gray-900 truncate">{resource.name}</h4>
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/50 p-5 sticky top-24 space-y-4">
              ...
              <div>
                <h4 className="text-xs font-bold text-gray-500 uppercase mb-3">
                  Online ({activityUsers.filter(u => u.lastActive === "Online").length})
                </h4>
                <div className="space-y-2">
                  {activityUsers
                    .filter(u => u.lastActive === "Online")
                    .map((activityUser) => (
                      <div key={activityUser.user.id} className="flex items-center gap-3 p-2 hover:bg-gray-50 rounded-xl transition cursor-pointer">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-green-600 font-medium">Đang online</p>
                        </div>
                      </div>
                    ))}
                </div>
                ...
                <div className="space-y-2">
                  {activityUsers
                    ...
                    .map((activityUser, index) => (
                      <div key={activityUser.user.id} className="flex items-center gap-3 p-2 bg-gradient-to-r from-orange-50 to-yellow-50 rounded-xl">
                        <div className="relative">
                          <Image
                            src={activityUser.user.avatar}
                            alt={activityUser.user.name}
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-gray-900 truncate">{activityUser.user.name}</p>
                  ...
                  {activityUsers
                    ...
                    .map((activityUser) => (
                      <div key={activityUser.user.id} className="flex items-center gap-3 p-2 hover:bg-gray-50 rounded-xl transition cursor-pointer">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-gray-900 truncate">{activityUser.user.name}</p>
                          <p className="text-xs text-gray-500">{activityUser.lastActive}</p>
                        </div>
                      </div>
                    ))}
                </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                />
                <div>
                  <h3 className="font-semibold text-gray-900">{currentUserData.name}</h3>
                  <p className="text-sm text-gray-500">Đang đăng trong: {event.title}</p>
                </div>
              </div>
              ...
              <div className="border-2 border-gray-200 rounded-xl p-4">
                  ...
                  <input
                    ...
                    onKeyPress={(e) => {
                      if (e.key === 'Enter' && imageUrlInput.trim()) {
                        setNewPostImages([...newPostImages, imageUrlInput.trim()]);
                        setImageUrlInput("");
                      }
                    }}
                  />
                ...
                <p className="text-xs text-gray-500 mt-2">Nhấn Enter hoặc nút Thêm để thêm ảnh</p>
              </div>
            </div>
      ...
      <div className="fixed bottom-8 right-8 z-50">
        {showFAB && (
          <div className="absolute bottom-20 right-0 bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/50 p-4 space-y-2 min-w-[200px] animate-slideUp">
            <button
              onClick={() => { setShowCreatePostModal(true); setShowFAB(false); }}
            ...
            >
              ...
              <span className="font-semibold text-gray-700 group-hover:text-green-600">Tạo bài viết</span>
            </button>
            <button
              onClick={() => { setActiveTab("chat"); setShowFAB(false); }}
              className="w-full flex items-center gap-3 px-4 py-3 hover:bg-blue-50 rounded-xl transition-all group"
            >
              <div className="p-2 bg-blue-100 rounded-lg group-hover:bg-blue-200 transition">
                <FaComments className="text-blue-600" />
              </div>
              <span className="font-semibold text-gray-700 group-hover:text-blue-600">Nhắn tin</span>
            </button>
            <button
              onClick={() => { setActiveTab("resources"); setShowFAB(false); }}
            ...
            >
              ...
              <span className="font-semibold text-gray-700 group-hover:text-purple-600">Tải tài liệu</span>
            </button>
          </div>
        )}
