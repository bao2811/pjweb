...
export default function ManagerEvents() {
    ...
    try {
      ...
      if (data && Array.isArray(data.events)) {
        ...
        const transformed: ManagerEvent[] = data.events.map((e: any) => {
          ...
          const now = new Date();
          let lifecycle: "upcoming" | "ongoing" | "completed" | "cancelled" =
            "upcoming";
          ...
          return {
            ...
            current_participants:
              ...
              0,
            participants_count:
              e.participants_count ?? e.current_participants ?? 0,
            approved_participants:
              e.approved_participants ?? e.participating_count ?? 0,
  ...
  const getLifecycleBadge = (status: ManagerEvent["lifecycleStatus"]) => {
    ...
    return (
      <span
        className={`flex items-center space-x-1 px-3 py-1 rounded-full text-sm font-semibold ${conf.classes}`}
      >
        {Icon ? <Icon /> : null}
  ...
  const getApprovalBadge = (status: ManagerEvent["approvalStatus"]) => {
    ...
    return (
      <span
        className={`inline-flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-semibold ${conf.classes}`}
      >
        {Icon ? <Icon /> : null}
        <span>{conf.label}</span>
      </span>
                  ...
                  <div className="space-y-2 mb-3 flex-1">
                    ...
                    <div className="flex flex-col gap-1 text-xs text-gray-500 mt-2 pt-2 border-t border-gray-100">
                      <div className="flex items-center justify-between">
                        <span className="text-xs" title="Ngày tạo">
                          ⚙️ {formatDate(event.created_at)}
                        </span>
