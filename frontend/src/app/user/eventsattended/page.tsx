...
interface UserEvent {
  ...
  userStatus: "pending" | "approved" | "participating" | "rejected" | "completed" | "cancelled";
...
export default function EventsAttendedPage() {
    ...
    const fetchUserEvents = async () => {
      try {
        ...
        if (response.data && Array.isArray(response.data)) {
          ...
          const transformedEvents: UserEvent[] = response.data
            ...
            .map((registration: any) => {
              ...
              return {
                ...
                organizer: {
                  ...
                  avatar: event.author?.avatar || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=150&h=150&fit=crop&crop=face',
                  role: 'manager',
                },
                participants: [],
                ...
                appliedAt: registration.created_at || new Date().toISOString(),
                approvedAt: registration.updated_at && registration.status === 'approved' ? registration.updated_at : undefined,
  ...
  const getEventStatus = (startTime: string, endTime: string): "upcoming" | "ongoing" | "completed" | "cancelled" => {
    if (!startTime) return 'upcoming';
    
    const now = new Date();
    const start = new Date(startTime);
    const end = endTime ? new Date(endTime) : new Date(start.getTime() + 4 * 60 * 60 * 1000); // Default 4h duration
  ...
  const handleCancelRegistration = async (eventId: number) => {
    ...
    try {
      ...
      await api.post(`/leaveEvent/${eventId}`);
  ...
  const filteredEvents = events.filter((event) => {
    ...
    if (selectedTab === "approved") {
      ...
      return matchesSearch && (event.userStatus === "approved" || event.userStatus === "participating");
    }
  ...
  const getTabCounts = () => {
    return {
      ...
      approved: events.filter((e) => e.userStatus === "approved" || e.userStatus === "participating").length,
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-xl p-4 border-2 border-yellow-200">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-yellow-800">{tabCounts.pending}</p>
                </div>
            ...
            <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-4 border-2 border-green-200">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-green-700 font-medium">Đang tham gia</p>
                  <p className="text-2xl font-bold text-green-800">{tabCounts.approved}</p>
                </div>
            ...
            <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-4 border-2 border-blue-200">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-blue-700 font-medium">Hoàn thành</p>
                  <p className="text-2xl font-bold text-blue-800">{tabCounts.completed}</p>
                </div>
            ...
            <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-4 border-2 border-purple-200">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-purple-700 font-medium">Tổng cộng</p>
                  <p className="text-2xl font-bold text-purple-800">{tabCounts.all}</p>
                </div>
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
            ...
            <nav className="flex space-x-2 bg-gray-100 rounded-xl p-1">
              {[
                { key: "all", label: "Tất cả", count: tabCounts.all, icon: FaCalendarAlt },
                { key: "pending", label: "Chờ duyệt", count: tabCounts.pending, icon: FaHourglassHalf },
                { key: "approved", label: "Đang tham gia", count: tabCounts.approved, icon: FaCheckCircle },
                { key: "rejected", label: "Bị từ chối", count: tabCounts.rejected, icon: FaTimesCircle },
                { key: "completed", label: "Hoàn thành", count: tabCounts.completed, icon: FaTrophy },
              ].map((tab) => {
                ...
                return (
                  ...
                  >
                    <TabIcon className={selectedTab === tab.key ? "text-blue-600" : "text-gray-400"} />
                    <span>{tab.label}</span>
                    <span
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    <div className="space-y-2 pt-4 border-t border-gray-100">
                      ...
                      {(event.userStatus === "approved" || event.userStatus === "participating") && (
                        <>
                          <button
