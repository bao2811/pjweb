...
export default function ManagerEventDetailPage({
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "members" | "evaluation" | "report">("overview");
  ...
  const handleDeleteEvent = async () => {
    ...
    try {
      ...
      const response = await authFetch(`/manager/events/${id}`, { method: "DELETE" });
      const data = await response.json();
      if (data.success) {
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-purple-200 mb-6">
          <div className="relative w-full aspect-video max-h-[400px]">
            <Image
              src={event.image || "https://images.unsplash.com/photo-1559027615-cd4628902d4a?w=1200&h=600&fit=crop"}
              alt={event.title}
              fill
          ...
          <div className="p-6">
            <div className="flex items-center space-x-2 mb-3">
              ...
              <span className={`px-3 py-1 rounded-full text-sm font-semibold ${
                ...
                event.status === "accepted" || event.status === "upcoming" ? "bg-green-100 text-green-700" :
                event.status === "ongoing" ? "bg-blue-100 text-blue-700" :
                event.status === "completed" ? "bg-gray-100 text-gray-700" :
                "bg-red-100 text-red-700"
              }`}>
                {event.status === "pending" ? "Chờ duyệt" :
                 event.status === "accepted" || event.status === "upcoming" ? "Đã duyệt" :
                 event.status === "ongoing" ? "Đang diễn ra" :
                 event.status === "completed" ? "Hoàn thành" : "Bị từ chối"}
              </span>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-4 mb-6 border border-purple-200">
          <div className="flex flex-wrap gap-3">
            <button onClick={handleEditEvent} className="flex items-center space-x-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition text-sm">
              <FaEdit /><span>Chỉnh sửa</span>
            </button>
            <button onClick={handleDeleteEvent} className="flex items-center space-x-2 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition text-sm">
              <FaTrash /><span>Xóa sự kiện</span>
            </button>
            {/* Chỉ hiển thị nút vào kênh chat khi sự kiện KHÔNG ở trạng thái pending (đã được admin duyệt) */}
            {event.status !== "pending" && event.status !== "rejected" ? (
              <button onClick={handleJoinChat} className="flex items-center space-x-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition text-sm">
                <FaComments /><span>Vào kênh chat</span>
              </button>
            ) : (
              <span className="flex items-center space-x-2 px-4 py-2 bg-gray-400 text-white rounded-lg text-sm cursor-not-allowed" title="Kênh chat chỉ mở sau khi sự kiện được duyệt">
                <FaComments /><span>Kênh chat (chờ duyệt)</span>
              </span>
            )}
          </div>
        </div>
          ...
          <div className="p-6">
            ...
            {activeTab === "overview" && (
              <div className="space-y-6">
                <div>
                  <h2 className="text-2xl font-bold text-gray-800 mb-4">Giới thiệu sự kiện</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex items-start space-x-3 p-4 bg-purple-50 rounded-lg">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-gray-800">
                        {event.start_date ? new Date(event.start_date).toLocaleString("vi-VN", {
                          day: "2-digit", month: "2-digit", year: "numeric",
                          hour: "2-digit", minute: "2-digit", hour12: false
                        }) : "Chưa xác định"}
                      </p>
                      <p className="text-sm text-gray-600">
                        đến {event.end_date ? new Date(event.end_date).toLocaleString("vi-VN", {
                          day: "2-digit", month: "2-digit", year: "numeric",
                          hour: "2-digit", minute: "2-digit", hour12: false
                        }) : "Chưa xác định"}
                      </p>
                  ...
                  <div className="flex items-start space-x-3 p-4 bg-purple-50 rounded-lg">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-gray-800">{event.location || "Chưa xác định"}</p>
                    </div>
                  </div>
                  <div className="flex items-start space-x-3 p-4 bg-purple-50 rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <span className="font-semibold text-gray-800">{event.currentParticipants || 0}/{event.max_participants}</span>
                        <span className="text-sm text-gray-600">{progress.toFixed(0)}%</span>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-3">
                        <div className="bg-gradient-to-r from-purple-500 to-purple-600 h-3 rounded-full transition-all duration-300" style={{ width: `${progress}%` }}></div>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-start space-x-3 p-4 bg-purple-50 rounded-lg">
            ...
            {activeTab === "members" && (
              <div className="space-y-4">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-2xl font-bold text-gray-800">Quản lý người tham gia</h2>
                  <div className="flex space-x-4">
                    <span className="px-3 py-1 bg-yellow-100 text-yellow-700 rounded-full text-sm font-semibold">Chờ duyệt: {pendingCount}</span>
                    <span className="px-3 py-1 bg-green-100 text-green-700 rounded-full text-sm font-semibold">Đã duyệt: {approvedCount}</span>
                  </div>
                </div>
                ...
                {participants.length === 0 ? (
                  ...
                  participants
                    ...
                    .map((participant) => (
                      <div key={participant.id} className={`p-4 rounded-xl border-2 ${
                        participant.status === "pending" ? "bg-yellow-50 border-yellow-200" : "bg-green-50 border-green-200"
                      }`}>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-4">
                            {participant.user.image ? (
                              <Image src={participant.user.image} alt={participant.user.username} width={50} height={50} className="rounded-full" unoptimized />
                            ) : (
                            ...
                            <div>
                              <p className="font-semibold text-gray-800">{participant.user.username}</p>
                          ...
                          <div className="flex items-center space-x-2">
                            {participant.status === "pending" ? (
                              <>
                                <button onClick={() => handleApproveUser(participant.user_id)} disabled={isProcessing} className="flex items-center space-x-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition disabled:opacity-50 text-sm">
                                  <FaUserCheck /><span>Duyệt</span>
                                </button>
                                <button onClick={() => handleRejectUser(participant.user_id)} disabled={isProcessing} className="flex items-center space-x-2 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition disabled:opacity-50 text-sm">
                                  <FaUserTimes /><span>Từ chối</span>
                                </button>
                              </>
                            ) : (
                              <span className="flex items-center space-x-2 px-4 py-2 bg-green-600 text-white rounded-lg text-sm">
                                <FaCheckCircle /><span>Đã duyệt</span>
                              </span>
                            )}
                          </div>
                        </div>
            ...
            {activeTab === "evaluation" && (
              <div className="space-y-4">
                <h2 className="text-2xl font-bold text-gray-800 mb-4">Đánh giá tình nguyện viên</h2>
                
                {approvedParticipants.length === 0 ? (
                  <p className="text-gray-500 text-center py-8">Chưa có tình nguyện viên được duyệt</p>
                ) : (
                  approvedParticipants.map((participant) => (
                    <div key={participant.id} className={`p-4 rounded-xl border-2 ${
                      participant.completion_status === "completed" ? "bg-green-50 border-green-300" :
                      participant.completion_status === "failed" ? "bg-red-50 border-red-300" :
                        ...
                        <div className="flex items-center space-x-4 flex-1">
                          ...
                          <div className="flex-1">
                            ...
                            {participant.completion_status === "completed" && (
                              <p className="text-xs text-green-600 mt-1">✅ Hoàn thành - {participant.completed_at ? new Date(participant.completed_at).toLocaleDateString("vi-VN") : ""}</p>
                            )}
                            {participant.completion_status === "failed" && (
                              <p className="text-xs text-red-600 mt-1">❌ Không đạt - {participant.completed_at ? new Date(participant.completed_at).toLocaleDateString("vi-VN") : ""}</p>
                            )}
                            {participant.completion_note && (
                              <p className="text-xs text-gray-500 mt-1 italic">Ghi chú: {participant.completion_note}</p>
                            )}
                          </div>
                        </div>
            ...
            {activeTab === "report" && report && (
              <div className="space-y-6">
                <h2 className="text-2xl font-bold text-gray-800 mb-4">Báo cáo chi tiết</h2>
                
                {/* Statistics */}
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="p-4 bg-blue-50 rounded-lg">
                    <p className="text-sm text-gray-600">Tổng đăng ký</p>
                    <p className="text-2xl font-bold text-blue-600">{report.statistics.total_registered}</p>
                  </div>
                  ...
                  <div className="p-4 bg-purple-50 rounded-lg">
                    ...
                    <p className="text-2xl font-bold text-purple-600">{report.statistics.completed}</p>
                  </div>
                  <div className="p-4 bg-orange-50 rounded-lg">
                    <p className="text-sm text-gray-600">Tổng giờ</p>
                    <p className="text-2xl font-bold text-orange-600">{report.statistics.total_volunteer_hours}h</p>
                  </div>
                </div>
                ...
                <div className="p-4 bg-gray-50 rounded-lg">
                  <div className="flex items-center justify-between mb-2">
                    <p className="font-semibold text-gray-700">Tỉ lệ hoàn thành</p>
                    <p className="font-bold text-gray-800">
                      {report.statistics.approved > 0
                        ? ((report.statistics.completed / report.statistics.approved) * 100).toFixed(1)
                        : 0}%
                    </p>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-4">
                    <div
                      ...
                      style={{
                        width: `${report.statistics.approved > 0 ? (report.statistics.completed / report.statistics.approved) * 100 : 0}%`,
                      }}
                    ></div>
                  </div>
                </div>
                ...
                {/* Volunteers List */}
                <div>
                  <h3 className="text-lg font-bold text-gray-800 mb-3">Danh sách tình nguyện viên</h3>
                    ...
                    <table className="w-full">
                      <thead className="bg-gray-100">
                        <tr>
                          ...
                          <th className="px-4 py-2 text-center text-sm font-semibold text-gray-700">Trạng thái</th>
                          <th className="px-4 py-2 text-center text-sm font-semibold text-gray-700">Hoàn thành</th>
                          <th className="px-4 py-2 text-center text-sm font-semibold text-gray-700">Giờ</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-200">
                        {report.volunteers.map((v) => (
                          <tr key={v.id} className="hover:bg-gray-50">
                            <td className="px-4 py-3 text-sm font-medium text-gray-800">{v.username}</td>
                            ...
                            <td className="px-4 py-3 text-center">
                              <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                                v.completion_status === "completed" ? "bg-purple-100 text-purple-700" :
                                v.completion_status === "failed" ? "bg-red-100 text-red-700" :
                                "bg-gray-100 text-gray-700"
                              }`}>
                                {v.completion_status === "completed" ? "Hoàn thành" :
                                 v.completion_status === "failed" ? "Không đạt" : "Chưa đánh giá"}
                              </span>
                            </td>
                            <td className="px-4 py-3 text-center text-sm font-semibold text-gray-800">{v.hours}h</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
