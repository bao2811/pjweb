...
export default function EventsAttendedPage() {
  ...
  const getTabCounts = () => {
    return {
      ...
      completed: events.filter((e) => e.completionStatus === "completed").length,
    };
  };
                      ...
                      {(event.userStatus === "approved" ||
                        ...
                        <>
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            ...
                            {event.eventStatus !== "completed" && event.eventStatus !== "ongoing" ? (
                              <button
                                onClick={() => handleCancelRegistration(event.id)}
                                disabled={cancellingEventId === event.id}
                              ...
                              <span className="flex items-center justify-center space-x-1 px-3 py-2 text-sm bg-gray-200 text-gray-400 rounded-lg font-medium cursor-not-allowed">
                                <FaCheckCircle />
                                <span>{event.eventStatus === "completed" ? "Đã kết thúc" : "Đang diễn ra"}</span>
                              </span>
                            )}
                          </div>
                        </>
